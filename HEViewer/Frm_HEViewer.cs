...
namespace HEViewer
{
    public partial class Frm_HEViewer : Form
    {
        ...
        /// 当前窗口中的he
        /// </summary>
        private HImageExt m_HImageExt = new HImageExt();
        /// <summary>
        /// 显示在窗体中的roi
        /// </summary>
        private List<MeasureROI> ROIlist = new List<MeasureROI>();
        /// <summary>
        /// 显示模式
        /// </summary>
        ...
        /// 当前he容器
        /// </summary>
        private Dictionary<string, HImageExt> heDic = new Dictionary<string, HImageExt>();

        /// <summary>
        /// 插入需要显示的he
        /// </summary>
        public void insertHE(string name, HImageExt he)
        {
            try
            {
                ...
                this.BeginInvoke(new Action(() =>
                {
                    ...
                    {
                        //当窗口最小化的时候 会错误 magical 20171030
        ...
        public void showHeByShowModel()
        {
            ...
            switch (showModel.ToString())
            {
                ...
                case "显示效果图":
                    showModel = enShowModel.显示效果图;
                    ROIlist = m_HImageExt.measureROIlist.ToList();//此处一定要使用toList
                    hWindowHE1.showHE(m_HImageExt);
                    break;
                case "显示检测范围":
                    showModel = enShowModel.显示检测范围;
                    ROIlist = m_HImageExt.measureROIlist.Where(c => enMeasureROIType.检测范围.ToString() == c.roiType).ToList();//此处一定要使用toList
                    break;
                case "显示检测点":
                    showModel = enShowModel.显示检测点;
                    ROIlist = m_HImageExt.measureROIlist.Where(c => enMeasureROIType.检测点.ToString() == c.roiType).ToList();//此处一定要使用toList
                    break;
                case "显示检测结果":
                    showModel = enShowModel.显示检测结果;
                    ROIlist = m_HImageExt.measureROIlist.Where(c => enMeasureROIType.检测结果.ToString() == c.roiType).ToList();//此处一定要使用toList
                    break;
                case "显示搜索范围":
                    showModel = enShowModel.显示搜索范围;
                    ROIlist = m_HImageExt.measureROIlist.Where(c => enMeasureROIType.搜索范围.ToString() == c.roiType).ToList();//此处一定要使用toList
                    break;
                default:
                    break;
            }
            ...
            //显示roi
            foreach (MeasureROI item in ROIlist)
            {
            ...
            {
                hWindowHE1.viewWindow.resetWindowImage();
                fitImageFlag = true;
            }
        }
        ...
        private void dataGridView_roi_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            try
            {
                if (e.RowIndex != -1)
                {
                    MeasureROI tempROi = m_HImageExt.measureROIlist[e.RowIndex];
                    ...
                    {
                        ROIlist.Add(m_HImageExt.measureROIlist[e.RowIndex]);//显示roi
                    }
        ...
        private void listBox_fileName_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                if (listBox_fileName.SelectedIndex != -1)
                {
                    ...
                    {//离线模式
                        m_HImageExt = SerialTool.FromBinary<HImageExt>(fileNameArr[listBox_fileName.SelectedIndex]);
                    }
                    ...
                    if (m_HImageExt != null)
                    {
                        ...
                        fitImageFlag = false;
                        showHeByShowModel();
                        dataGridView_roi.DataSource = m_HImageExt.measureROIlist;
                    }
                }
            }
            catch (Exception ex)
            ...
            if (dialog.ShowDialog() == DialogResult.OK)
            {
                ...
                ThreadPool.QueueUserWorkItem(o =>
                {
                    ...
                    for (int i = 0; i < fileNameArr.Length; i++)
                    {
                        ...
                        HImageExt tempHE = SerialTool.FromBinary<HImageExt>(fileNameArr[i]);
        ...
        public void saveWindowDumpByPng(HImageExt tempHE, string fileName, Size size)
        {
            try
            {
